"""
ReceptivIQ — Layered Stack Architecture (Development Environment)

设计意图：
- 严格自上而下的 7 层架构图：Presentation → Infrastructure
- 每层 = 一个水平 band，左侧 Layer N 标签，右侧工具节点
- LangChain 用于 Agent 编排，httpx 处理直连 LLM 调用
- Coolify 作为底层 PaaS 控制平面，虚线管控边界覆盖所有 Docker 服务
- 5 色数据流箭头：用户 / AI / 数据 / 合规 / 部署

输出：dev-stack-layered.{svg, png}
"""
...
OUT_SVG = Path("dev-stack-layered.svg")
OUT_PNG = Path("dev-stack-layered.png")
...
parts.append(
    ...
    f'ReceptivIQ Platform &#8212; 开发环境架构 (Dev Stack)</text>'
)
parts.append(
...
parts.append(
    ...
    f'部署平台: Coolify (图形化 PaaS)  |  '
    f'后端: Python/FastAPI  |  前端: React  |  '
    f'存储: PostgreSQL 3-Lake (Landing + Raw PII + Processed · 本地 schema 模拟)'
)
parts.append('</text>')
parts.append(env_badge(CR - 220, 28, '⚠ DEV', '开发环境（本地）', '#F59E0B'))
...
# Compliance 已上移为横切大框（见文件底部）；此处不再渲染小盒
...
# ── L5 重构：3-Lake（dev：本地 PG schema 模拟）+ Redis + MinIO ──
EDGE_PII_LOCAL = "#EF4444"
BRONZE_LOCAL = "#A16207"
NODE_W = 130

# 🟫 Landing Lake (Bronze · dev = postgres schema `landing`)
parts.append(tool_node(L5_X0,        L5_Y, "postgresql", "Landing Lake", "Bronze · 整条 record", accent=BRONZE_LOCAL, w=NODE_W))
# 软分隔（同 PII trust boundary · dev 仅是同实例 schema 切分）
...
# 🔴 Raw PII Lake
parts.append(tool_node(L5_X0 + NODE_W + 22, L5_Y, "postgresql", "🔒 Raw PII Lake", "PII 字段 only", accent=EDGE_PII_LOCAL, w=NODE_W))
# PII Boundary 粗红虚线
...
# 🟢 Processed Lake (含 pgvector for AI 召回)
parts.append(tool_node(L5_X0 + 2 * (NODE_W + 22) + 32, L5_Y, "postgresql", "✓ Processed Lake", "dbt 5 层 + pgvector", accent=LAYER_COLORS[5], w=NODE_W))
parts.append(tool_node(L5_X0 + 3 * (NODE_W + 22) + 32, L5_Y, "redis",      "Redis",           "Cache + broker", accent=LAYER_COLORS[5], w=NODE_W))
parts.append(tool_node(L5_X0 + 4 * (NODE_W + 22) + 32, L5_Y, "minio",      "MinIO",           "S3 storage",     accent=LAYER_COLORS[5], w=NODE_W))

# tool quality tags
parts.append(tool_tag(L5_X0 + 18,                          L5_Y + 116, "dev: PG schema", BRONZE_LOCAL))
parts.append(tool_tag(L5_X0 + (NODE_W + 22) + 18,          L5_Y + 116, "Fernet 加密",     EDGE_PII_LOCAL))
...
parts.append(
    ...
    f' fill="{LABEL}">物理隔离 · per-Agency DB + KMS</text>'
)
# Row 2: Client = RLS in Agency
parts.append(
...
# Data Sources（按类汇总，避免与 L4 EXTRACT ADAPTERS 重复）
DS_X = L6_X0 + 750
...
# ═══════════════════════════════════════════════════════════
# LAYER 7 — Infrastructure / Deployment （Coolify 重点）
# ═══════════════════════════════════════════════════════════
parts.append(band_bg(7))
parts.append(layer_label(7, "部署层 ⭐", "Infrastructure", "Coolify PaaS"))
...
if icon:
    ...
    # fallback: render the SVG path inline
    parts.append(
...
# Docker + GitHub
parts.append(tool_node(L7_X0 + 320, L7_Y, "docker", "Docker", "容器运行时", accent=LAYER_COLORS[7], h=120))
...
# 2) Coolify 管控边界：L2–L7（内框，不与 Compliance 冲突）
ctrl_top = band_y(2) - 4
ctrl_bot = band_y(7) - 4
...
parts.append(
    ...
    f'ReceptivIQ Platform &#183; Layered Stack v1.0  &#183;  Generated by dev-stack-layered.py</text>'
)
