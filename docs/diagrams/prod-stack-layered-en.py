"""
ReceptivIQ — Production Stack Architecture — English

Design intent:
- Strict top-down 7-layer architecture: Presentation → Infrastructure
- PRODUCTION stack: Neon Postgres (3-Lake: Landing + Raw PII + Processed), Redis Cloud, AWS S3, Render (managed PaaS)
- Render as managed PaaS control plane (auto-scaling, managed databases, TLS, GitHub deploys)
- 5-color data flow arrows: User / AI / Data / Compliance / Deploy

Output: prod-stack-layered-en.{svg, png}
"""
...
OUT_SVG = Path("prod-stack-layered-en.svg")
OUT_PNG = Path("prod-stack-layered-en.png")
...
parts.append(
    ...
    f'ReceptivIQ Platform &#8212; Production Stack Architecture</text>'
)
parts.append(
    f'<text x="{(CR+PAD)//2}" y="80" font-family="Helvetica" font-size="14"'
    f' fill="{SUBTITLE}" text-anchor="middle">'
    f'7-layer production stack  &#183;  Render-managed deployment  &#183;  '
...
parts.append(
    ...
    f'Deployment: Render (managed PaaS)  |  '
    f'Backend: Python/FastAPI  |  Frontend: React  |  '
    f'Storage: Neon 3-Lake (per-Agency: Landing + Raw PII + Processed) + AWS S3'
)
parts.append('</text>')
parts.append(env_badge(CR - 220, 28, '✓ PROD', 'PRODUCTION ENV', '#10B981'))
...
# Compliance moved to right-side panel
...
# ── L5 redesign: 3-Lake · Landing + Raw PII + Processed + Redis + S3 ──
EDGE_PII_LOCAL = "#EF4444"
BRONZE_LOCAL = "#A16207"
NODE_W = 130

# 🟫 Landing Lake (Bronze) — STAGE 1 full-record landing
parts.append(tool_node(L5_X0,        L5_Y, "neon", "Landing Lake", "Bronze · full record", accent=BRONZE_LOCAL, w=NODE_W))
...
# 🔴 Raw PII Lake — STAGE 3 derive A
parts.append(tool_node(L5_X0 + NODE_W + 22, L5_Y, "neon", "🔒 Raw PII Lake", "PII fields only", accent=EDGE_PII_LOCAL, w=NODE_W))
# PII Boundary thick red dashed (only between Raw PII and Processed)
...
# 🟢 Processed Lake — STAGE 3 derive B + STAGE 4 dbt
parts.append(tool_node(L5_X0 + 2 * (NODE_W + 22) + 32, L5_Y, "neon", "✓ Processed Lake", "dbt 5 layers · no PII", accent=LAYER_COLORS[5], w=NODE_W))
# Redis / S3
parts.append(tool_node(L5_X0 + 3 * (NODE_W + 22) + 32, L5_Y, "redis",  "Redis Cloud", "Managed cache + broker", accent=LAYER_COLORS[5], w=NODE_W))
parts.append(tool_node(L5_X0 + 4 * (NODE_W + 22) + 32, L5_Y, "aws_s3", "AWS S3",      "Cloud object storage", accent=LAYER_COLORS[5], w=NODE_W))
...
parts.append(
    ...
    f' fill="{LABEL}">Physical · per-Agency project + KMS</text>'
)
parts.append(
...
parts.append(layer_label(7, "Infrastructure", "Render Managed PaaS", "Production Deployment"))
...
parts.append(
    ...
    f' fill="{LABEL}" font-weight="600">Managed Cloud PaaS</text>'
)
for i, line in enumerate([
    "Auto-deploy from GitHub",
    "Auto-scaling + zero TLS",
    "Managed PG + Redis",
...
parts.append(tool_node(L7_X0 + 320, L7_Y, "docker", "Docker", "Build runtime", accent=LAYER_COLORS[7], h=120))
...
ctrl_items = [
    ...
    "Cron jobs", "Web Service (Airflow)", "Worker (Airflow sched)",
...
parts.append(
    ...
    f'ReceptivIQ Platform &#183; Production Stack v1.0 (EN)  &#183;  Generated by prod-stack-layered-en.py</text>'
)
