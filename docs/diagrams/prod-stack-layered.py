"""
ReceptivIQ — 生产环境分层架构（Production Stack）

设计意图：
- 严格自上而下的 7 层架构图：Presentation → Infrastructure
- 生产栈：Neon Postgres（3-Lake：Landing + Raw PII + Processed）+ Redis Cloud + AWS S3 + Render（托管 PaaS）
- Render 作为托管 PaaS 控制平面（自动部署 + 自动扩缩容 + 托管数据库）
- 5 色数据流箭头：用户 / AI / 数据 / 合规 / 部署

输出：prod-stack-layered.{svg, png}
"""
...
OUT_SVG = Path("prod-stack-layered.svg")
OUT_PNG = Path("prod-stack-layered.png")
...
# 数据流颜色
EDGE_USER   = "#06B6D4"   # cyan — 用户流量
EDGE_AI     = "#EC4899"   # pink — LLM 调用
EDGE_DATA   = "#10B981"   # emerald — 数据入仓
EDGE_GUARD  = "#A855F7"   # purple — 合规/审计
EDGE_DEPLOY = "#F59E0B"   # amber — 部署管控
...
# v2: production cool/professional theme override
LAYER_COLORS = {
    1: "#0EA5E9",   # Presentation — sky blue
    2: "#14B8A6",   # API Gateway — teal
    3: "#6366F1",   # Application — indigo
    4: "#059669",   # ELT — deep emerald
    5: "#64748B",   # Data — slate (cool gray)
    6: "#3B82F6",   # External — blue
    7: "#10B981",   # Infrastructure — emerald
}

# 画布尺寸
W, H = 2320, 1500
CR = 1960       # 原内容右边界（保持既有布局）
PAD = 40
LAYER_LABEL_W = 180
...
def layer_label(layer: int, cn_name: str, en_name: str, description: str) -> str:
    """左侧分层标签"""
    y_top = band_y(layer)
  ...
  <g>
    ...
    <text x="{cx}" y="{cy - 4}" font-family="Helvetica" font-size="18"
          font-weight="800" fill="{TITLE}" text-anchor="middle">{cn_name}</text>
    <text x="{cx}" y="{cy + 18}" font-family="Helvetica" font-size="11"
          fill="{DIM_LABEL}" text-anchor="middle">{en_name}</text>
...
parts.append(layer_label(2, "接入层", "API Gateway", "HTTP/WS + Middleware"))
...
parts.append(layer_label(3, "应用服务层", "Application", "AI + Business Logic"))
...
# ═══════════════════════════════════════════════════════════
# LAYER 4 — Transformation / ELT （重点层）
# ═══════════════════════════════════════════════════════════
parts.append(band_bg(4))
parts.append(layer_label(4, "ELT 管道 ⭐", "Extract-Load-Transform", "先保存后转换"))
...
# 编排引擎可选：Airflow（current）/ Dagster（target，per ADR-003）
...
parts.append(tool_node(L4_X0 + 286,  L4_Y + 14, "celery",       "Celery",      "异步: PDF / 受众 / 告警", accent=LAYER_COLORS[4], h=130, w=130))
...
# Compliance 已迁移至右侧侧栏面板

# dbt 转换
DBT_Y = L4_Y + 150
parts.append(
    f'<rect x="{L4_X0 - 4}" y="{DBT_Y - 14}" width="170" height="20" fill="{BG}"'
    f' stroke="{LAYER_COLORS[4]}" stroke-width="1" rx="6"/>'
    f'<text x="{L4_X0 + 81}" y="{DBT_Y + 1}" font-family="Helvetica" font-size="11"'
    f' font-weight="700" fill="{LAYER_COLORS[4]}" text-anchor="middle"'
    f' letter-spacing="1">转换 (dbt SQL 管理)</text>'
)
parts.append(tool_node(L4_X0, DBT_Y + 10, "dbt", "dbt Core", "清洗·合并·映射·聚合", accent=LAYER_COLORS[4], h=55, w=160))
# dbt layer pipeline
dbt_layers = [
    ("staging",   "清洗", "字段标准化"),
    ("canonical", "合并", "跨平台统一"),
...
parts.append(layer_label(5, "数据层", "Data Storage", "OLTP + OLAP + Cache"))

L5_Y = band_y(5) + 25
L5_X0 = PAD + LAYER_LABEL_W + 60

# ── L5 重构：3-Lake · Landing + Raw PII + Processed + Redis + S3 ──
EDGE_PII_LOCAL = "#EF4444"
BRONZE_LOCAL = "#A16207"
NODE_W = 130

# 🟫 Landing Lake (Bronze) — STAGE 1 整条 record 着陆点
parts.append(tool_node(L5_X0,        L5_Y, "neon", "Landing Lake", "Bronze · 整条 record", accent=BRONZE_LOCAL, w=NODE_W))
...
parts.append(
    ...
    f' fill="#94A3B8" text-anchor="middle" opacity="0.85">同 PII zone</text>'
)
# 🔴 Raw PII Lake — STAGE 3 派生 A
parts.append(tool_node(L5_X0 + NODE_W + 22, L5_Y, "neon", "🔒 Raw PII Lake", "PII 字段 only", accent=EDGE_PII_LOCAL, w=NODE_W))
# PII Boundary 粗红虚线（仅 Raw PII ↔ Processed 之间）
...
# 🟢 Processed Lake — STAGE 3 派生 B + STAGE 4 dbt
parts.append(tool_node(L5_X0 + 2 * (NODE_W + 22) + 32, L5_Y, "neon", "✓ Processed Lake", "dbt 5 层 · 无 PII", accent=LAYER_COLORS[5], w=NODE_W))
# Redis / S3
parts.append(tool_node(L5_X0 + 3 * (NODE_W + 22) + 32, L5_Y, "redis",  "Redis Cloud", "Managed cache", accent=LAYER_COLORS[5], w=NODE_W))
parts.append(tool_node(L5_X0 + 4 * (NODE_W + 22) + 32, L5_Y, "aws_s3", "AWS S3",      "Object storage", accent=LAYER_COLORS[5], w=NODE_W))

# tool tags 重新定位（3-Lake）
parts.append(tool_tag(L5_X0 + 18,                          L5_Y + 116, "HIPAA 6y · 90d",  BRONZE_LOCAL))
parts.append(tool_tag(L5_X0 + (NODE_W + 22) + 18,          L5_Y + 116, "per-Agency KMS",  EDGE_PII_LOCAL))
parts.append(tool_tag(L5_X0 + 2 * (NODE_W + 22) + 50,      L5_Y + 116, "RLS per Client",  LAYER_COLORS[5]))
parts.append(tool_tag(L5_X0 + 3 * (NODE_W + 22) + 50,      L5_Y + 116, "managed",         "#10B981"))
parts.append(tool_tag(L5_X0 + 4 * (NODE_W + 22) + 50,      L5_Y + 116, "cloud",           "#10B981"))

# ── TENANT ISOLATION 突出徽章（右侧）──
...
parts.append(
    ...
    f' fill="{LABEL}">物理隔离 · per-Agency project + KMS</text>'
)
parts.append(
    ...
    f' fill="{LABEL}">逻辑隔离 · client_id RLS (in-Agency)</text>'
)
...
parts.append(layer_label(6, "外部服务层", "External Services", "LLM + Observability + Sources"))

L6_Y = band_y(6) + 8
L6_X0 = PAD + LAYER_LABEL_W + 60

# LLM —— 双 Gateway：OpenRouter（默认）+ AWS Bedrock（HIPAA · BAA）
parts.append(section_title(L6_X0, L6_Y, "LLM STACK", EDGE_AI))
parts.append(tool_node(L6_X0, L6_Y + 14, "openrouter", "OpenRouter", "默认网关 · 多厂商路由", accent=EDGE_AI, h=48, w=140))
parts.append(tool_node(L6_X0, L6_Y + 70, "bedrock",    "AWS Bedrock", "HIPAA · BAA · Claude 托管", accent=EDGE_AI, h=48, w=140))
...
parts.append(
    ...
    f' fill="{DIM_LABEL}" font-style="italic">详细 9 平台 → 见 L4 EXTRACT ADAPTERS</text>'
)

# ═══════════════════════════════════════════════════════════
# LAYER 7 — Infrastructure / Render PaaS
# ═══════════════════════════════════════════════════════════
parts.append(band_bg(7))
parts.append(layer_label(7, "部署层 ⭐", "Render Managed PaaS", "生产部署"))
...
RND_X = L7_X0
parts.append(
    f'<rect x="{RND_X}" y="{L7_Y}" width="280" height="120" fill="{NODE_BG}"'
    f' stroke="{LAYER_COLORS[7]}" stroke-width="3" rx="14"/>'
)
parts.append(
    f'<rect x="{RND_X}" y="{L7_Y}" width="280" height="6" fill="{LAYER_COLORS[7]}" rx="3"/>'
)
icon = img_b64("render")
if icon:
    parts.append(f'<image href="{icon}" x="{RND_X + 16}" y="{L7_Y + 24}" width="80" height="80"/>')
parts.append(
    f'<text x="{RND_X + 116}" y="{L7_Y + 38}" font-family="Helvetica" font-size="22"'
    f' font-weight="800" fill="{LAYER_COLORS[7]}">Render</text>'
)
parts.append(
    f'<text x="{RND_X + 116}" y="{L7_Y + 58}" font-family="Helvetica" font-size="11"'
    f' fill="{LABEL}" font-weight="600">托管云 PaaS</text>'
)
for i, line in enumerate([
    ...
    parts.append(
        f'<text x="{RND_X + 116}" y="{L7_Y + 74 + i*12}" font-family="Helvetica"'
        f' font-size="10" fill="{DIM_LABEL}">{line}</text>'
    )

# Docker + GitHub
parts.append(tool_node(L7_X0 + 320, L7_Y, "docker", "Docker", "构建运行时", accent=LAYER_COLORS[7], h=120))
parts.append(tool_node(L7_X0 + 500, L7_Y, "github", "GitHub", "源码 + webhook", accent=LAYER_COLORS[7], h=120))

# Render 部署的服务
CTRL_X = L7_X0 + 700
parts.append(section_title(CTRL_X, L7_Y + 4, "RENDER-DEPLOYED SERVICES", LAYER_COLORS[7]))
ctrl_items = [
    "Web Service (FastAPI)", "Worker (Celery)", "Static Site (React)",
    "Cron 任务", "Web (Airflow)", "Worker (Airflow)",
    "→ Neon 3-Lake", "→ AWS S3", "→ Redis Cloud",
]
...
# ═══════════════════════════════════════════════════════════
# 跨层数据流箭头 — 走 margin 高速通道避免穿过工具
# ═══════════════════════════════════════════════════════════
LEFT_HW_X  = PAD + LAYER_LABEL_W + 32     # 252 — 在 layer 标签与工具之间
RIGHT_HW_X = CR - 340                # 1620 — L1 USER TYPES 之前，其他层右侧空白
...
# ─── ORM 写入 L3 业务服务 → L5 PostgreSQL（走右侧高速通道）
orm_start_x = PAD + LAYER_LABEL_W + 60 + 1170
orm_end_x   = PAD + LAYER_LABEL_W + 60 + 0 + 80
y3_bot = band_y(3) + BAND_HEIGHTS[3] - 6
y_l4_l5_gap = band_y(5) - 4
parts.append(
...
# ─── (1) EXTRACT — L6 数据源 → L4 adapters（从 L3-L4 间隙进入 adapter 顶部）
src_x_right = 1436                                      # 最后一个数据源 chip 右侧
src_y = 1097                                            # 数据源 row 0 中央
y_l3_l4_gap = 630                                       # L3-L4 间隙（626-634 中央）
adapter_top_x = 853                                     # HubSpot row 0 中心
adapter_top_y = 658                                     # adapter row 0 上方 2px (top=660)
parts.append(
...
# ─── (2) LOAD raw_* — adapter 右侧 → 绕过 dbt 右侧 → L5 header
adapter_exit_x = 1070                                   # adapter chips 右侧 9px 间隙 (chips end 1066, compliance starts 1075)
adapter_exit_y = 789                                    # row 1 底部下方 1px (row 1 ends 788)
y_horiz_below_dbt_title = 802                           # dbt 标题文字 (786-798) 下方、boxes (806+) 上方
dbt_right_x = 1450                                      # Field Mapping (ends 1140) 右侧 + buffer
y_l5_header = 876                                       # L5 header (PG/DuckDB top 887 上方)
warehouse_x = 540                                       # DuckDB 列中心
warehouse_top = 883                                     # DuckDB 顶部上方 4px
parts.append(
...
# ─── httpx → LLM（L3 agent → L6 OpenRouter，走左侧高速通道）
agent_exit_x   = PAD + LAYER_LABEL_W + 60 + 200
agent_exit_y   = band_y(3) + 12 - 4
y_l2_l3_gap    = band_y(3) - 4
openrouter_x   = PAD + LAYER_LABEL_W + 60 + 60
openrouter_y   = band_y(6) + 10 + 50
parts.append(
    f'<path d="M {agent_exit_x} {agent_exit_y} '
    f'L {agent_exit_x} {y_l2_l3_gap} '
    f'L {LEFT_HW_X} {y_l2_l3_gap} '
    f'L {LEFT_HW_X} {openrouter_y} '
    f'L {openrouter_x} {openrouter_y}" '
...
# ═══════════════════════════════════════════════════════════
# Compliance 右侧侧栏 + 显式连接到每一层（横切关注，全局必备）
# ═══════════════════════════════════════════════════════════
COMP_X = CR + 40
COMP_W = W - COMP_X - 30
SPINE_X = COMP_X - 16        # 卡片左外侧，确保连线不进框

# 1) 显式连接：从每一层 band 右边引出虚线 → 进入 spine（视觉证明"连接所有功能"）
...
parts.append(
    ...
    f'ReceptivIQ Platform &#183; Production Stack v1.0  &#183;  Generated by prod-stack-layered.py</text>'
)
...
# ═══════════════════════════════════════════════════════════
# Convert SVG → PNG (try multiple tools, fall back gracefully)
# ═══════════════════════════════════════════════════════════
converters = [
...
if not converted:
    print(f"! PNG conversion skipped (install rsvg-convert / inkscape / imagemagick to enable)")
